 ...
 * - 内部坐标：SSD1680 RAM 映射坐标，X[0-151], Y[0-296]
 * - 自动转换：用户坐标 → 内部坐标（转置）
 */
typedef struct {
 ...
 *         - ESP_OK: 成功
 *         - ESP_ERR_INVALID_ARG: 参数错误
 * @note 当字符串超出 max_width 时会自动换行（按单词换行，不会切断单词）
 ...
 * @param str 字符串
 * @param font 字体对象指针
 * @return uint16_t 字符串宽度（像素）
 */
uint16_t epd_get_string_width(const char *str, const epd_font_t *font);

/**
 * @brief 获取字符串高度（多行文本的总高度）
 * 
 * @param str 字符串（可包含换行符）
 * @param font 字体对象指针
 * @param max_width 最大宽度（像素），用于计算换行
 * @return uint16_t 字符串总高度（像素）
 */
